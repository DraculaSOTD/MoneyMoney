...
class DataAggregator:
    ...
    def get_raw_data(self, symbol: str, profile_id: int,
        ...
        """
        Fetch raw 1-minute data from database.
    ...
    def get_aggregated_data(self, symbol: str, profile_id: int,
        ...
        if start_time is None:
            # Calculate start_time based on limit and timeframe to ensure enough data
            if limit and timeframe in self.TIMEFRAME_MINUTES:
                minutes_needed = limit * self.TIMEFRAME_MINUTES[timeframe]
                # Add 10% buffer for partial candles and timezone differences
                start_time = end_time - timedelta(minutes=int(minutes_needed * 1.1))
            else:
                # Fallback defaults based on timeframe
                if timeframe == '1D':
                    start_time = end_time - timedelta(days=limit or 30)
                elif timeframe == '1M':
                    start_time = end_time - timedelta(days=(limit or 12) * 31)
                elif timeframe == '1h':
                    start_time = end_time - timedelta(hours=limit or 168)
                else:
