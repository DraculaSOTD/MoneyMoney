from sqlalchemy import create_engine, Column, String, Float, Integer, DateTime, Boolean, Enum as SQLEnum, JSON, Index, ForeignKey, Text
...
class ModelPerformanceMetrics(Base):
    ...
    # Time Window
    timestamp = Column(DateTime, default=datetime.utcnow, index=True)
    evaluation_period = Column(String)  # '1h', '24h', '7d', '30d'
    ...
    # Signal Accuracy
    signal_accuracy = Column(Float, nullable=True)  # % correct BUY/SELL/HOLD signals
    buy_signal_accuracy = Column(Float, nullable=True)
    sell_signal_accuracy = Column(Float, nullable=True)
    total_signals = Column(Integer, default=0)
    correct_signals = Column(Integer, default=0)

    # Direction Accuracy
    direction_accuracy = Column(Float, nullable=True)  # % correct up/down predictions
    up_predictions = Column(Integer, default=0)
    down_predictions = Column(Integer, default=0)
    correct_directions = Column(Integer, default=0)

    # Trading Performance (if signals acted upon)
    win_rate = Column(Float, nullable=True)
    ...
    # Error Distribution
    error_stats = Column(JSON)  # {min, max, std, percentiles}
    prediction_distribution = Column(JSON)  # Distribution of predictions vs actuals

    # Model Version
    model_version = Column(String, nullable=True)

    __table_args__ = (
        Index('ix_performance_profile_time', 'profile_id', 'timestamp'),
        Index('ix_performance_model_time', 'model_id', 'timestamp'),
        Index('ix_performance_period', 'profile_id', 'evaluation_period', 'timestamp'),
    )
