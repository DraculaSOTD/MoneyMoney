...
class BinanceConnector:
    ...
    async def get_all_historical_data(self, symbol: str, interval: str = '1m',
                                     start_date: Optional[datetime] = None,
                                     end_date: Optional[datetime] = None,
                                     progress_callback: Optional[Callable[[int, int, str], None]] = None) -> pd.DataFrame:
        """
        Fetch ALL available historical data with pagination.
        Handles Binance's 1000 candle limit per request by making multiple requests.
        ...
        # Estimate total candles if start_date provided
        total_estimated = None
        if start_ts:
            total_estimated = int((end_ts - start_ts) / interval_ms)
            logger.info(f"Estimated {total_estimated:,} candles to fetch for {symbol}")
        ...
        while True:
            ...
            try:
                ...
                logger.info(f"Batch {batch_count}: Fetched {len(batch_klines)} candles for {symbol} (total: {len(all_klines):,})")

                # Progress callback
                if progress_callback:
                    progress_callback(
                        len(all_klines),
                        total_estimated or len(all_klines),
                        f"Fetched {len(all_klines):,} candles (batch {batch_count})"
                    )

                # Check if we've reached the start date or beginning of available data
                oldest_time = data[0][0]  # First candle's open time in milliseconds
                if start_ts and oldest_time <= start_ts:
                    logger.info(f"Reached start date for {symbol}")
                    break

                # If we got less than limit, we've reached the beginning
                if len(data) < limit:
                    logger.info(f"Reached beginning of available data for {symbol}")
                    break

                # Update end timestamp for next request (use oldest candle's open time - 1ms)
                current_end_ts = oldest_time - 1

                # Rate limiting: Binance allows 1200 requests/min, so ~50ms between requests is safe
                await asyncio.sleep(0.05)

            except Exception as e:
                logger.error(f"Error fetching batch {batch_count} for {symbol}: {e}")
                raise
