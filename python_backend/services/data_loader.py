...
from python_backend.database.models import MarketData, TradingProfile
...
class DataLoader:
    ...
    def load_from_database(
        ...
        """
        Load OHLCV data from python_backend.database.
        ...
        try:
            ...
            logger.info(f"Loaded {len(df)} candles from python_backend.database for {symbol}")

            return df

        except Exception as e:
            logger.error(f"Error loading data from python_backend.database for {symbol}: {e}")
            raise
    ...
    def get_latest_candles(
        ...
        Args:
            symbol: Trading symbol
            profile_id: Profile ID (required if loading from python_backend.database)
        ...
        """
        try:
            # Try loading from python_backend.database first
